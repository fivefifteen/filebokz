(function (global, factory) {
  ...
  */
  ...
  var _filebokz = function filebokz(elements, applyClass) {
    ...
    if (!_filebokz.count) _filebokz.count = 0;
    ...
    var displayErrorMessage = function displayErrorMessage(fileBox, errorType, variable) {
      ...
      _filebokz.triggerEvent('error', fileBox, {
    ...
    var onChange = function onChange(e, fileBox, input) {
      ...
      if (files.length) {
        ...
        if (appendable && multiple && !removing && originalFiles.length) {
          ...
          input.files = _filebokz.newFileList(newFilesArr);
          files = input.files;
        }
      ...
      if (!files.length && !removing) {
        input.files = _filebokz.newFileList(originalFiles);
        files = input.files;
      }
      if (!files.length) {
      ...
      if (files.length > originalFiles.length) {
        _filebokz.triggerEvent('file-added', fileBox);
      } else if (files.length < originalFiles.length) {
        _filebokz.triggerEvent('file-removed', fileBox);
      }
    };
    ...
    var onDrop = function onDrop(e, fileBox, input) {
      ...
      if (oldFileBoxId) {
        ...
        if (validateFiles(filesArr, fileBox, input)) {
          files = _filebokz.newFileList(filesArr);
          oldFilesArr.splice(oldFileIndex, 1);
          oldInput.files = _filebokz.newFileList(oldFilesArr);
          _filebokz.triggerEvent('change', oldInput, {
      ...
      if (files && files.length) {
        input.files = files;
        _filebokz.triggerEvent('change', input);
      }
    };
    ...
    var onFileRemoveClick = function onFileRemoveClick(e, input, index) {
      e.preventDefault();
      e.stopPropagation();
      _filebokz.removeFiles(input, index);
    };
    ...
    for (var i = 0; i < elements.length; i++) {
      (function (fileBox) {
        ...
        _filebokz.count++;
        fileBox.dataset.filebokzId = _filebokz.count;
        ...
        if (!fileBox.id) fileBox.id = 'filebokz-' + _filebokz.count;
  ...
  _filebokz.addFiles = function (input, files) {
    var filesArr = Array.from(input.files);
    filesArr.concat(Array.from(files));
    input.files = _filebokz.newFileList(filesArr);
    _filebokz.triggerEvent('change', input);
  };
  _filebokz.newFileList = function (files) {
    var dt;
    try {
      dt = new DataTransfer();
    } catch (e) {
  ...
  _filebokz.removeFiles = function (input, index, count) {
    var filesArr = Array.from(input.files);
  ...
  _filebokz.triggerEvent = function (type, element, data, delay) {
